...
// netlify/functions/ai-chat-streaming.js
...
async function getConversationContext(userId, sessionId) {
  try {
    const { data } = await supabase.from("conversation_contexts").select("context_data").eq("user_id", userId).eq("session_id", sessionId).order("last_message_at", { ascending: false }).limit(1).single();
...
async function getUserEmotionProfile(userId) {
  try {
    const { data: recentDiaries } = await supabase.from("diaries").select("emotion, mood_score, ai_insights, created_at").eq("user_id", userId).order("created_at", { ascending: false }).limit(10);
    ...
    const primaryEmotion = Object.keys(emotionCounts).reduce(
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 200,
        stream: false
        // Netlify Functions는 스트리밍 제한이 있어 일반 응답 사용
      })
    });
...
function evaluateResponseQuality(response, userMessage) {
  ...
  if (empathyKeywords.some((keyword) => response.includes(keyword)))
    score += 1;
...
\uC9C0\uAE08 \uC774 \uC21C\uAC04\uC774 \uD798\uB4E4\uB354\uB77C\uB3C4, \uBC18\uB4DC\uC2DC \uC88B\uC544\uC9C8 \uB0A0\uC774 \uC62C \uAC83\uC785\uB2C8\uB2E4. \uC804\uBB38 \uC0C1\uB2F4\uC0AC\uC640 \uC774\uC57C\uAE30\uD574\uBCF4\uC138\uC694.`,
    ...
    const context2 = await getConversationContext(user.id, sessionId);
    context2.messages.push({
      role: "user",
      content: message,
      timestamp: (/* @__PURE__ */ new Date()).toISOString()
    });
    const systemPrompt = createPersonalizedSystemPrompt(emotionProfile, context2.conversation_depth || 0);
    const aiResponse = await generateStreamingResponse(context2.messages, systemPrompt);
    ...
    context2.messages = context2.messages.slice(-20);
    context2.conversation_depth = (context2.conversation_depth || 0) + 1;
    context2.emotion_state = emotionProfile.primary_emotion;
    await updateConversationContext(user.id, sessionId, context2);
    ...
    await supabase.from("chat_history").insert(historyInserts);
    if (emotionProfile.needs_support && Math.random() > 0.7) {
      const { data: recommendations } = await supabase.rpc("get_emotion_recommendations", {
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        conversation_depth: context2.conversation_depth,
        response_quality: responseQuality,
        processing_time: Date.now() - startTime,
        model_used: "gpt-4o-mini"
      })
...
//# sourceMappingURL=ai-chat-streaming.js.map
