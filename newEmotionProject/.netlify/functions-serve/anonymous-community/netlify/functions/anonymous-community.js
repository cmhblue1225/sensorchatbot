...
var require_helpers2 = __commonJS({
  "node_modules/@supabase/supabase-js/dist/main/lib/helpers.js"(exports2) {
    ...
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.validateSupabaseUrl = exports2.applySettingDefaults = exports2.isBrowser = exports2.ensureTrailingSlash = exports2.uuid = void 0;
    function uuid() {
...
var require_ethereum = __commonJS({
  "node_modules/@supabase/auth-js/dist/main/lib/web3/ethereum.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.createSiweMessage = exports2.toHex = exports2.fromHex = exports2.getAddress = void 0;
...
// netlify/functions/anonymous-community.js
...
function generateAnonymousId(userId) {
  const hash = import_crypto.default.createHash("sha256").update(userId + "anonymous_salt_2025").digest("hex");
...
async function getPersonalizedPosts(userId, limit = 10) {
  try {
    const { data: userEmotions } = await supabase.from("diaries").select("emotion, mood_score").eq("user_id", userId).order("created_at", { ascending: false }).limit(5);
    const userEmotionTypes = userEmotions?.map((d) => d.emotion) || [];
    const avgMoodScore = userEmotions?.length > 0 ? userEmotions.reduce((sum, d) => sum + (d.mood_score || 0), 0) / userEmotions.length : 0;
    let query = supabase.from("anonymous_posts").select(`
        *,
        comment_count,
        like_count
  ...
  try {
    ...
    if (event.httpMethod === "GET") {
      ...
      if (action === "posts") {
        let posts;
        if (emotion) {
          const { data } = await supabase.from("anonymous_posts").select("*").eq("emotion_category", emotion).order("created_at", { ascending: false }).limit(limit);
          posts = data || [];
        } else {
      ...
      if (action === "comments" && postId) {
        const { data: comments } = await supabase.from("anonymous_comments").select("*").eq("post_id", postId).order("created_at", { ascending: true });
        return {
          statusCode: 200,
          headers,
          body: JSON.stringify({
      ...
      if (action === "my_posts") {
        const { data: myPosts } = await supabase.from("anonymous_posts").select("*").eq("user_id", user.id).order("created_at", { ascending: false });
        return {
          statusCode: 200,
          headers,
          body: JSON.stringify({
    ...
    if (event.httpMethod === "POST") {
      ...
      if (action === "create_post") {
        ...
        const { data: insertedPost, error: insertError } = await supabase.from("anonymous_posts").insert(newPost).select().single();
        if (insertError) {
      ...
      if (action === "create_comment") {
        ...
        const { data: originalPost } = await supabase.from("anonymous_posts").select("emotion_category").eq("id", postId).single();
        if (!originalPost) {
          return {
            statusCode: 404,
            headers,
        ...
        const { data: insertedComment, error: commentError } = await supabase.from("anonymous_comments").insert(newComment).select().single();
        if (commentError) {
          throw new Error("\uB313\uAE00 \uC0DD\uC131 \uC2E4\uD328: " + commentError.message);
        }
        await supabase.from("anonymous_posts").update({ comment_count: supabase.sql`comment_count + 1` }).eq("id", postId);
        return {
          statusCode: 201,
          headers,
          body: JSON.stringify({
      ...
      if (action === "like_post") {
        ...
        const { data: updatedPost } = await supabase.from("anonymous_posts").update({ like_count: supabase.sql`like_count + 1` }).eq("id", postId).select().single();
        return {
          statusCode: 200,
          headers,
          body: JSON.stringify({
    ...
    if (event.httpMethod === "PUT") {
      ...
      if (action === "view_post" && postId) {
        await supabase.from("anonymous_posts").update({ view_count: supabase.sql`view_count + 1` }).eq("id", postId);
        return {
          statusCode: 200,
          headers,
...
//# sourceMappingURL=anonymous-community.js.map
