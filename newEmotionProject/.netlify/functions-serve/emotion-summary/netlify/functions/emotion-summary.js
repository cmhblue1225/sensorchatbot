...
// netlify/functions/emotion-summary.js
...
exports.handler = async (event, context) => {
  ...
  try {
    if (event.httpMethod === "POST") {
      const { emotions: emotions2 } = JSON.parse(event.body || "{}");
      if (!emotions2 || !Array.isArray(emotions2)) {
        return {
          statusCode: 400,
          headers,
      ...
      const validEmotions = emotions2.filter((e) => e.score !== null);
    ...
    const { data: emotions, error } = await supabase.from("diaries").select("emotion, created_at").eq("user_id", user.id).order("created_at", { ascending: false });
    if (error) {
      return {
        statusCode: 500,
        headers,
...
//# sourceMappingURL=emotion-summary.js.map
