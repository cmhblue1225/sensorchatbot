...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const publicSupabase = (0, import_supabase_js.createClient)(
      process.env.SUPABASE_URL || "https://vzmvgyxsscfyflgxcpnq.supabase.co",
    ...
    const { error: diariesError } = await supabase.from("diaries").delete().eq("user_id", user.id);
    if (diariesError) {
      console.error("\uC77C\uAE30 \uC0AD\uC81C \uC624\uB958:", diariesError);
    }
    const { error: chatError } = await supabase.from("chat_history").delete().eq("user_id", user.id);
    if (chatError) {
