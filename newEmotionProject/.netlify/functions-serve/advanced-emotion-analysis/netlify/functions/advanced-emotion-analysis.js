...
// netlify/functions/advanced-emotion-analysis.js
...
async function getCachedAnalysis(contentHash, analysisType) {
  try {
    const { data } = await supabase.from("ai_analysis_cache").select("output_data, confidence_score").eq("content_hash", contentHash).eq("analysis_type", analysisType).gt("expires_at", (/* @__PURE__ */ new Date()).toISOString()).single();
    return data;
  } catch (error) {
    return null;
  }
...
async function findSimilarEmotions(diaryId, userId, limit = 5) {
  try {
    const { data: embeddingData } = await supabase.from("emotion_embeddings").select("embedding").eq("diary_id", diaryId).single();
    if (!embeddingData)
      return [];
    const { data: similarEntries } = await supabase.rpc("find_similar_emotions", {
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const contentHash = import_crypto.default.createHash("sha256").update(content + user.id).digest("hex");
    ...
    const { data: recentEmotions } = await supabase.from("diaries").select("emotion, mood_score, created_at").eq("user_id", user.id).order("created_at", { ascending: false }).limit(5);
    const userContext = {
    ...
    if (diaryId) {
      ...
      if (embedding) {
        await supabase.from("emotion_embeddings").insert({
          diary_id: diaryId,
          embedding,
    ...
    if (analysis.emotions && analysis.emotions.length > 0) {
      ...
      await supabase.from("emotion_patterns").upsert({
        user_id: user.id,
        pattern_type: "daily",
...
//# sourceMappingURL=advanced-emotion-analysis.js.map
